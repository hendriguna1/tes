<?php 
           ...
           // $stdevy= Stand_Deviation($nilaiY);
            ...
            $CovarintS =0;
           // $angkaacak = rand(3,98);
            $bilacak = [0.5751,0.1270,0.7039,0.3853,0.9166,0.2888,0.9518,0.7348,0.1347,0.9014];
           ...
           // die();
            ...
            $hari = 1;
            for ($c=0; $c<COUNT($bilacak1); $c++)      
            {
                $nilaiSimulasi = explode("|", Interval_value($bilacak1[$c]));
                $totalSimulasi += $nilaiSimulasi[1];
                
                $hasilu2 = $totalSimulasi/10;
                $hasil2 .= '<tr>
                        <td>'.$hari.' </td>
                        <td>'.$bilacak[$c].'</td>
